        ...
        public void Text(float x, float y, string text, FntFontData font, Color tint, float scaleX=1, float scaleY=1, float z=0)
        {
            ...
            foreach (var ch in text.Replace(Environment.NewLine, "\n"))
            {
                ...
                Vertex(new VertexPosTexColor {
                    PosX = x + (fntChar.XOffset + xOffset) * scaleX, PosY = y + (fntChar.YOffset + yOffset) * scaleY, PosZ = z,
                    TexCoordX = fntChar.X / scaleW, TexCoordY = fntChar.Y / scaleH,
                    ColorR = tint.R, ColorG = tint.G, ColorB = tint.B, ColorA = tint.A });

                Index();
                Vertex(new VertexPosTexColor {
                    PosX = x + (fntChar.XOffset + xOffset + fntChar.Width) * scaleX, PosY = y + (fntChar.YOffset + yOffset) * scaleY, PosZ = z,
                    TexCoordX = (fntChar.X + fntChar.Width) / scaleW, TexCoordY = fntChar.Y / scaleH,
                    ColorR = tint.R, ColorG = tint.G, ColorB = tint.B, ColorA = tint.A });

                Index();
                Vertex(new VertexPosTexColor {
                    PosX = x + (fntChar.XOffset + xOffset + fntChar.Width) * scaleX, PosY = y + (fntChar.YOffset + yOffset + fntChar.Height) * scaleY, PosZ = z,
                    TexCoordX = (fntChar.X + fntChar.Width) / scaleW, TexCoordY = (fntChar.Y + fntChar.Height) / scaleH,
                    ColorR = tint.R, ColorG = tint.G, ColorB = tint.B, ColorA = tint.A });
                ...
                Vertex(new VertexPosTexColor {
                    PosX = x + (fntChar.XOffset + xOffset) * scaleX, PosY = y + (fntChar.YOffset + yOffset + fntChar.Height) * scaleY, PosZ = z,
                    TexCoordX = fntChar.X / scaleW, TexCoordY = (fntChar.Y + fntChar.Height) / scaleH,
                    ColorR = tint.R, ColorG = tint.G, ColorB = tint.B, ColorA = tint.A });
